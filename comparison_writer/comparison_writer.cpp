...
int main( int argc, char* argv[] )
{
  ...
  printf( "Dataspaces Writer:  N( %d ) M( %d ) nrepeat ( %d ) problem( %g MB ) time( %g s ) bandwidth( %g GB/s )\n",
  ...
  for (int timestep = 0; timestep < nrepeat; timestep++)
  {
      std::string path = "cppio_rank";
      path += std::to_string(rank) + "_ts";
      path += std::to_string(timestep) + ".dat";
      std::ofstream output;
      output.open(path, std::ios::out | std::ios::binary);
      output.write(reinterpret_cast<char *>(A.data()), sizeof(double)*N*M);
      output.close();
  }
  double time1 = timer1.seconds();
  printf( "CppIO Writer:  N( %d ) M( %d ) nrepeat ( %d ) problem( %g MB ) time( %g s ) bandwidth( %g GB/s )\n",
          N, M, nrepeat, Gbytes * 1000, time1, Gbytes * nrepeat / time1 );

  Kokkos::Timer timer2;

  std::string filepath = "HDf5_rank";
  ...
  for (int timestep = 0; timestep < nrepeat; timestep++)
  {
      ...
      hid_t dataset_id = H5Dcreate2(file_id, path.c_str(), H5T_NATIVE_DOUBLE, dataspace_id,
                                    H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

      herr_t dataset_status = H5Dwrite(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, A.data());
  ...
  printf( "HDF5 Writer:  N( %d ) M( %d ) nrepeat ( %d ) problem( %g MB ) time( %g s ) bandwidth( %g GB/s )\n",
